...
label1:
	...
	lw	$t1 16($t1)
	jalr		$t1
	la	$a0 int_const0   // push int 8 on to stack
	sw	$a0 0($sp)
	addiu	$sp $sp -4
	la	$a0 int_const1   // push int 7 on to stack
	sw	$a0 0($sp)
	addiu	$sp $sp -4
	lw	$a0 12($s0)      // load A Object
	bne	$a0 $zero label2
	la	$a0 str_const0
	li	$t1 1
	jal	_dispatch_abort
label2:
	lw	$t1 8($a0)       // A_dispTab
	lw	$t1 12($t1)      // A.f
	jalr		$t1      // jump and link A.f
	lw	$fp 12($sp)
	lw	$s0 8($sp)
	lw	$ra 4($sp)
	addiu	$sp $sp 12
